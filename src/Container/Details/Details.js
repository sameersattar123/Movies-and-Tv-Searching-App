...
const Details = () => {
  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(
        `https://api.themoviedb.org/3/${_media_type}/${id}?api_key=d3ccacc37598c63c0097decf085283a4&language=en-US`
      );
      setContent(data);
    } catch (error) {
      console.log(error);
  ...
  const fetchVideos = async () => {
    try {
      const { data } = await axios.get(
        `https://api.themoviedb.org/3/${_media_type}/${id}/videos?api_key=d3ccacc37598c63c0097decf085283a4&language=en-US`
      );
  ...
  const fetchCredits = async () => {
    try {
      const { data } = await axios.get(
        `https://api.themoviedb.org/3/${_media_type}/${id}/credits?api_key=d3ccacc37598c63c0097decf085283a4&language=en-US`
      );
                ...
                <div  className='contentHead'>
                    <Container>
                        <Row>
                            <Col className='col-12'>
                                {
                                    credits && credits.length > 0 ? <Carsousel data={credits} /> : 'Lading data...'
                                }
